...
and mu (the mean) and sigma (the random error) are defined as:

$\mu = alpha + \beta_w*weight + \beta_s*sport + \beta_{ws}*weight*sport$
$\sigma = x$

where x is any value we define. Typically, we might make this also drawn from a random distribution, but here we will set it to a fixed value. Below, we will set the parameters as follows:
